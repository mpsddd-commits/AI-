...
if __name__ == "__main__":
  ...
  # 수정 후: properties 내의 None 값을 제거하는 전처리 추가
  processed_rels = []
  for rel in data.get("relationships", []):
      if "properties" in rel and rel["properties"]:
          # 값이 None인 항목들을 필터링하여 제외
          rel["properties"] = {k: v for k, v in rel["properties"].items() if v is not None}
      processed_rels.append(Neo4jRelationship(**rel))
