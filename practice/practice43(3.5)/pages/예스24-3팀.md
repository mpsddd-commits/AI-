  ...
  try:    
    ...
    if res.status_code == 200:
      ...
      for item in trs:
        ...
        star = "0.0"
        if star_span:
          star_no = star_span.select_one("em.yes_b")
          star = star_no.get_text(strip=True) if star_no else star_span.get_text(strip=True)
        ...
        if categoryNumber == "006":
          books.append({ "상품명": title, "명칭": author, "별점": star, "sale_num": sale_num  })
        elif categoryNumber == "003":
          books.append({ "CD/LP": title, "아티스트": author, "별점": star, "sale_num": sale_num  })
        elif categoryNumber == "004":
          books.append({ "DVD/BD": title, "감독 / 제작사": author, "별점": star, "sale_num": sale_num  })
        else:
          books.append({ "도서명": title, "저자": author, "별점": star, "sale_num": sale_num  })
      
      if len(books_db) > 0:
        if categoryNumber == "017&eBookTp=0":
          categoryNumber = "017"
        # 서버PC 등록
        sql1 = f"DELETE FROM edu.`yes24` WHERE `category` = '{categoryNumber}' AND `week_no` = {weekNo}"
        sql2 = f"""
          ...
          """
        values = [
          (weekNo, book["title"], book["author"], book["star"], categoryNumber, book["sale_num"] )
